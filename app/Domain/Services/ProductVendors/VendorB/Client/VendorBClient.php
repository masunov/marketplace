...
namespace App\Domain\Services\ProductVendors\VendorB\Client;
...
readonly class VendorBClient
{
    ...
    public function issueKey(string $orderId, string $sku, string $requestId): IssueKeyResponse
    {
        ...
        if (Lottery::odds($this->backendErrorChance, $this->totalChances)->choose()) {
            Log::error(
                'Vendor B server error while issuing key',
                [
                    'order_id'   => $orderId,
                    'sku'        => $sku,
                    'request_id' => $requestId,
                    'vendor'     => VendorEnum::VENDOR_B->value,
                    'reason'     => 'Server error'
                ]
            );
        ...
        if ($key) {
            ...
            if (Lottery::odds($this->timeoutErrorChance, $this->totalChances - $this->backendErrorChance)->choose()) {
                Log::error(
                    'Vendor B timeout while issuing key',
                    [
                        'order_id'   => $orderId,
                        'sku'        => $sku,
                        'request_id' => $requestId,
                ...
                throw new TimeoutException('Timeout from vendor B in issueKey request exception');
            }
        ...
        Log::info(
            'Vendor B out_of_stock while issuing key',
            [
                'order_id'   => $orderId,
                'sku'        => $sku,
                'request_id' => $requestId,
    ...
    public function checkKey(string $orderId, string $requestId): IssueKeyResponse
    {
        ...
        if (Lottery::odds($this->backendErrorChance, $this->totalChances)->choose()) {
            Log::error(
                'Vendor B server error while checking key',
                [
                    'order_id'   => $orderId,
                    'request_id' => $requestId,
                    'vendor'     => VendorEnum::VENDOR_B->value,
        ...
        $key = ExternalVendorKey::findByRequestId(VendorEnum::VENDOR_B, $requestId);
        ...
        Log::info(
            'Vendor B confirms key was not issued',
            [
                'order_id'   => $orderId,
                'request_id' => $requestId,
                'vendor'     => VendorEnum::VENDOR_B->value,
